...
const IAModale: React.FC<IAModaleProps> = ({ onClose }) => {
    ...
    const [featuresRecommendations, setFeaturesRecommendations] = useState<
        string[]
    >([]);
        ...
        try {
            ...
            if (step === 1) {
                ...
                const recommendationJSON =
                    JSON.parse(
                        response_part1
                    ); /* Parse les recommendations de l'IA */
                ...
                setFrontendStacks(
                    recommendationJSON.recommendations.frontend
                ); /* Stocke les stacks frontend */
                setBackendStacks(
                    recommendationJSON.recommendations.backend
                ); /* Stocke les stacks backend */
                setDatabaseStacks(
                    recommendationJSON.recommendations.database
                ); /* Stocke les stacks database */
                ...
                const content2 = {
                    Frontend: finalChoiceFrontend,
                    Backend: finalChoiceBackend,
                    Database: finalChoiceDatabase,
                };
                const response_part2 = await updateThread(threadId, content2); // Request BuildIT IA
                const featuresRecommendationsJSON =
                    JSON.parse(
                        response_part2
                    ); /* Parse les recommendations de l'IA */
                const featuresRecommendationsArray: string[] = Object.values(
                    featuresRecommendationsJSON.features_recommendations
                ) as string[];
                setFeaturesRecommendations(
                    featuresRecommendationsArray
                ); /* Stocke les features recommandées */
            ...
            <div className="relative w-3/5 bg-bgPrimary p-6 rounded-lg shadow-lg max-w-lg">
                ...
                {loading && (
                    <div className="flex flex-col items-center justify-center p-10">
                        ...
                        <div>
                            <h2 className="text-white text-3xl mt-20 mb-20 font-thin text-center">
                ...
                {!loading && step === 2 && (
                    <>
                        ...
                        <div className="flex flex-col gap-4">
                            <h3 className="text-primary font-bold text-lg mt-5">
                                Frontend Stack
                            </h3>
                            <div className="frontend-stack-select flex">
                                {frontendStacks.map((stack) => (
                                    <div
                                        key={stack}
                                        className={`card-stack w-1/4 text-center bg-bgSecondary p-3 rounded-lg shadow-lg ml-3 text-wrap hover:bg-secondary hover:scale-105 transition-all cursor-pointer ${
                                            frontend === stack
                            ...
                            {frontend === "Other" && (
                                <input
                                    onChange={(e) =>
                                        setFrontendCustomStack(e.target.value)
                                    }
                            ...
                            {backend === "Other" && (
                                <input
                                    onChange={(e) =>
                                        setBackendCustomStack(e.target.value)
                                    }
                            ...
                            {database === "Other" && (
                                <input
                                    onChange={(e) =>
                                        setDatabaseCustomStack(e.target.value)
                                    }
                    ...
                    <>
                        ...
                        <div className="flex flex-col gap-4 h-80 p-4 rounded-lg overflow-y-auto">
                            {featuresRecommendations.map((feature) => (
                                <div
                                    key={feature}
                                    className={`features-card bg-bgSecondary p-4 rounded-lg shadow-lg text-wrap break-words hover:bg-secondary hover:scale-105 transition-all cursor-pointer ${
                ...
                {!loading && step === 5 && (
                    <div className="flex flex-col items-center justify-center p-10">
                        ...
                        <h2 className="text-white text-3xl mt-10 mb-20 font-thin text-center">
